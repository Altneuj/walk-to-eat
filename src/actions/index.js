...
export function fetchRestaurants(query, current) {
  const config = {
    ...
    q: query, in: `${current.latitude},${current.longitude};r=10000`
  }

  const request = axios.get(`${ROOT_URL_RESTAURANTS}`, {params: config});
  return {type: FETCH_RESTAURANTS, payload: request};
}
...
export function fetchCurrent() {
  ...
  return {type: FETCH_CURRENT, payload: getPosition()}
}

export function resetRestaurants(list) {
  return {type: SHORTENED_LIST, payload: list}
}
